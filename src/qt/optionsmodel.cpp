...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeGoCoinMeAmount"))
        settings.setValue("nAnonymizeGoCoinMeAmount", 1000);

    nAnonymizeGoCoinMeAmount = settings.value("nAnonymizeGoCoinMeAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeGoCoinMeAmount"))
        SoftSetArg("-anonymizegocoinmeamount", settings.value("nAnonymizeGoCoinMeAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in gocoinme.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeGoCoinMeAmount:
            return QVariant(nAnonymizeGoCoinMeAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeGoCoinMeAmount:
            nAnonymizeGoCoinMeAmount = value.toInt();
            settings.setValue("nAnonymizeGoCoinMeAmount", nAnonymizeGoCoinMeAmount);
            emit anonymizeGoCoinMeAmountChanged(nAnonymizeGoCoinMeAmount);
            break;
        case CoinControlFeatures:
