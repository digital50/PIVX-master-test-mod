...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a GoCoinMe address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no GoCoinMe: URI
...
{
    // Convert gocoinme:// to gocoinme:
    //
    //    Cannot handle this later, because gocoinme:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open gocoinme.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "GoCoinMe.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for GoCoinMe.lnk
...
{
    return GetAutostartDir() / "gocoinme.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a gocoinme.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=GoCoinMe\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the gocoinme app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add gocoinme app to startup item list
